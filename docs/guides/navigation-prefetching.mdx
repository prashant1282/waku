...
- `errorTtl` sets how long a failed prefetch is left alone before another trigger retries it, in milliseconds (default: 10000). Without it, a route that cannot be prefetched — one that answers with a document location, say — would refetch on every hover. Set it to `0` to retry on every trigger.
